...
1. Load the `PG008_original.xlsx` file into a data.frame object, create a factor version of the `year` variable, and determine if the mean fork length of fish sampled from the "Seawater" `water.type` differs among years.  [*Note, be careful about the inconsistent use of levels in `water.type`*.]
```{r message=FALSE, warning=FALSE}
...
2. [*Time permitting*]  Determine if the mean fork length of **Age-15 to 20** (inclusive) Arctic Charr captured in **"freshwater"** differs among years.
```{r message=FALSE, warning=FALSE}
